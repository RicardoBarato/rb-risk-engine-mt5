...
function Find-Mt5ByOrigin {
    param(
        [string] $PreferredOrigin = "C:\Program Files\Tickmill MT5 Terminal - Scalper"
    )
    ...
    $scalper = $matches | Where-Object { $_.InstallDir -like "*Scalper*" } | Select-Object -First 1
    if ($scalper) {
        return @{
            InstallDir = $scalper.InstallDir
            DataDir    = $scalper.DataDir
        }
    }

    $tickmill = $matches | Where-Object { $_.InstallDir -like "*Tickmill*" } | Select-Object -First 1
    if ($tickmill) {
        return @{
            InstallDir = $tickmill.InstallDir
            DataDir    = $tickmill.DataDir
        }
    }
...
function Get-EaSourcePath {
    $repoRoot = Get-RepoRoot
    return Join-Path $repoRoot "MQL5\Experts\RoboScalper\RoboScalper.mq5"
}

function Get-EaTargetPath {
    param([string] $DataDir)

    return Join-Path $DataDir "MQL5\Experts\RoboScalper\RoboScalper.mq5"
}
