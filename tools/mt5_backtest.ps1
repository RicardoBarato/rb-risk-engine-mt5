param(
    ...
    [string] $Server = "",
    [switch] $CompileFirst,
    [string] $ExpertPath = "RBRiskEngine\RBRiskEngine_Public",
    [string] $SourcePath = "",
    [string] $SetPath = "",
...
$mt5RunRoot = Join-Path $paths.DataDir "MQL5\Files\RBRiskEngineRuns"
...
if ($SetPath -eq "" -and $ExpertPath -eq "RBRiskEngine\RBRiskEngine_Public") {
    $SetPath = Join-Path $repoRoot "examples\RBRiskEngine_Public.example.set"
